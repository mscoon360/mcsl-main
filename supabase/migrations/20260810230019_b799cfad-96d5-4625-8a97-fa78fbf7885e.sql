...
BEGIN
  ...
  IF gross > 0 THEN
    ...
    LOOP
      ...
      WHERE m.product_id = it.product_id
      ...
      IF line_rev <> 0 THEN
        entries_json := entries_json || jsonb_build_object(
          ...
          'meta', jsonb_build_object('sale_id', s.id, 'sale_item_id', it.id, 'product_id', it.product_id)
        );
      END IF;
...
AS $$
BEGIN
  PERFORM public.rebuild_sale_ledger(NEW.id);
  RETURN NEW;
END;
$$;

CREATE OR REPLACE FUNCTION public.rebuild_sale_ledger_from_item()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
BEGIN
  PERFORM public.rebuild_sale_ledger(COALESCE(NEW.sale_id, OLD.sale_id));
  RETURN COALESCE(NEW, OLD);
END;
$$;

DROP TRIGGER IF EXISTS rebuild_sale_ledger_on_item ON public.sale_items;
CREATE TRIGGER rebuild_sale_ledger_on_item
AFTER INSERT OR UPDATE OR DELETE ON public.sale_items
FOR EACH ROW EXECUTE FUNCTION public.rebuild_sale_ledger_from_item();